...
def _set_moire(n_moire: int, symm: str='d3', rot:float=0) -> tuple:
    ...
    # genarate atomic structure
    # first `a_` represents for atomic
    atomic_structure = Structure(symm, rot)
    a_unitvec_1 = atomic_structure.a_unitvec_1
    a_unitvec_2 = atomic_structure.a_unitvec_2
    a_g_unitvec_1 = atomic_structure.a_g_unitvec_1
    a_g_unitvec_2 = atomic_structure.a_g_unitvec_2
    ...
    # moire length scale
    m_l_theta = A_C*np.sqrt(3*n_moire**2+3*n_moire+1)
    m_k_theta = 4*np.pi/(3*m_l_theta)
    m_basis_vecs = {}
    m_basis_vecs['mu1'] = m_unitvec_1
    m_basis_vecs['mu2'] = m_unitvec_2
    m_basis_vecs['mg1'] = m_g_unitvec_1
    m_basis_vecs['mg2'] = m_g_unitvec_2
    m_basis_vecs['ltheta'] = m_l_theta
    m_basis_vecs['ktheta'] = m_k_theta
    high_symm_pnts = {}
...
def set_atom_pstn_list(n_moire: int, corru: bool=True, symm: str='d3', rot: float=0) -> np.ndarray:
    ...
    print("Generate Data with symm and rot:", symm, rot)
    (rt_angle_r, _), m_basis_vecs, _ = _set_moire(n_moire, symm, rot)
    m_g_unitvec_1 = m_basis_vecs['mg1']
    m_g_unitvec_2 = m_basis_vecs['mg2']
    m_l_theta = m_basis_vecs['ltheta']
    rt_mtrx_half = _set_rt_mtrx(rt_angle_r/2)
    ...
    if symm=='d3':
        atom_a_pstn = atomic_structure.atom_pstn_1
        atom_b_pstn = atomic_structure.atom_pstn_2-atomic_structure.a_unitvec_1
    else:
        atom_a_pstn = atomic_structure.atom_pstn_1-atomic_structure.a_unitvec_1
        atom_b_pstn = atomic_structure.atom_pstn_2-atomic_structure.a_unitvec_1
    ...
    #     atom_a_pstn = ATOM_PSTN_1
    #     atom_b_pstn = ATOM_PSTN_2-A_UNITVEC_1
    # else:
    #     ATOM_PSTN_1 = Structure.ATOM_PSTN_1_D6
    #     ATOM_PSTN_2 = Structure.ATOM_PSTN_2_D6 
    #     atom_a_pstn = ATOM_PSTN_1-A_UNITVEC_1
    #     atom_b_pstn = ATOM_PSTN_2-A_UNITVEC_1
    
    small_g_vec = np.array([m_g_unitvec_1, m_g_unitvec_2, -m_g_unitvec_1-m_g_unitvec_2])

    # searching boundary
    ly = m_l_theta/2*np.sqrt(3)
    n = int(2*ly/A_C)+2
    delta = 0.0001
    ...
    for (ix, iy) in product(range(n), range(n)):
        atom_pstn = -ix*a_unitvec_1+iy*a_unitvec_2+atom_a_pstn
