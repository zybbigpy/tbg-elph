...
def _set_g_vec_list_valley(n_moire: int, g_vec_list: np.ndarray, m_basis_vecs: dict,
    ...
    offset_1 = (n_moire)*(m_g_unitvec_1+m_g_unitvec_2)
    offset_2 = -n_moire*(m_g_unitvec_1+m_g_unitvec_2)
    gvc = np.append(g_vec_list+offset_1, g_vec_list+offset_2, axis=0)
...
def _cal_eigen_hamk(hamk, smat, datatype=DataType.CORRU, engine=EngineType.TBPLW) -> tuple:
    ...
    """
    w = 0
    print("check close for hamk", np.allclose(np.conj(hamk), hamk))
    #hamk = np.real(hamk)
...
def tb_solver(n_moire: int,
    ...
    if datatype == DataType.RELAX or datatype == DataType.PHONON:
        # load atom data
        atom_pstn_list = mio.read_atom_pstn_list(n_moire, datatype)
        # construct moire info
        (_, m_basis_vecs, high_symm_pnts) = mset._set_moire(n_moire, symm=symm, rot=rot)
    else:
        # generate atom data
        if datatype == DataType.RIGID:
            print("save d6 structure")
            atom_pstn_list = mset.set_atom_pstn_list(n_moire, corru=False, symm=symm, rot=rot)
            np.savetxt("atom30_d6_rigid.csv", atom_pstn_list, delimiter=",")
        else:
            atom_pstn_list = mset.set_atom_pstn_list(n_moire, corru=True, symm=symm, rot=rot)
        
        (_, m_basis_vecs, high_symm_pnts) = mset._set_moire(n_moire, symm, rot)
