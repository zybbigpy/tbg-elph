...
def tb_plot_sparsetb(n_moire: int, n_g: int, n_k: int, bands: int, datatype: str, pathname="./", figname="", mu=False):
    fig, ax = plt.subplots()
    ret = mtb.tb_solver(n_moire, n_g, n_k, True, symm='d3', rot=0, datatype=datatype, engine=EngineType.TBSPARSE)
    emesh = ret['emesh']
    kline = ret['kline']
...
def tb_plot_fulltb(n_moire: int, n_g: int, n_k: int, bands: int, datatype: str, pathname="./", figname="", mu=False):
    fig, ax = plt.subplots()
    ret = mtb.tb_solver(n_moire, n_g, n_k, True, symm='d3', rot=0, datatype=datatype, engine=EngineType.TBFULL)
    emesh = ret['emesh']
    kline = ret['kline']
...
def tb_plot_tbplw_sepv(n_moire: int, n_g: int, n_k: int, bands: int, datatype: str, pathname="./", figname="",symm='d3', rot=0,
                       mu=False):
    fig, ax = plt.subplots()
    ret = mtb.tb_solver(n_moire, n_g, n_k, True, symm=symm, rot=rot, datatype=datatype, valley=ValleyType.VALLEYK1)
    emesh = ret['emesh']
    kline = ret['kline']
    band_plot_module(ax, kline, emesh, n_k, bands, figname=figname, mu=mu)
    ret = mtb.tb_solver(n_moire, n_g, n_k, True, symm=symm, rot=rot, datatype=datatype, valley=ValleyType.VALLEYK2)
    emesh = ret['emesh']
    kline = ret['kline']
    band_plot_module(ax, kline, emesh, n_k, bands, figname=figname, mu=mu)
    plt.tight_layout()
    plt.savefig(pathname+"moire_"+str(n_moire)+"_"+datatype+symm+"_tbplw_sepv.png", dpi=500)
    plt.show()
    plt.close()
...
def tb_plot_tbplw_combv(n_moire: int,
                        ...
                        mu=False):
    fig, ax = plt.subplots()
    ret = mtb.tb_solver(n_moire, n_g, n_k, disp=True, symm=symm,  rot= rot, datatype=datatype,  valley=ValleyType.VALLEYC)
    emesh = ret['emesh']
    kline = ret['kline']
    band_plot_module(ax, kline, emesh, n_k, bands, figname=figname, mu=mu)
    plt.tight_layout()
    #plt.savefig(pathname+"moire_"+str(n_moire)+"_"+datatype+"_"+symm+"_rot_"+str(int(rot))+"_tbplw_combv.png", dpi=500)
    plt.show()
    plt.close()
...
def fulltb_sepv_cmp(n_moire: int,
                    ...
                    figname=""):
    fig, ax = plt.subplots()
    ret = mtb.tb_solver(n_moire, n_g, n_k1, True, symm='d3', rot=0, datatype=datatype, valley=ValleyType.VALLEYK1)
    emesh = ret['emesh']
    kline = ret['kline']
    band_plot_module(ax, kline, emesh, n_k1, band1, figname=figname)
    ret = mtb.tb_solver(n_moire, n_g, n_k1, True, symm='d3', rot=0, datatype=datatype, valley=ValleyType.VALLEYK2)
    emesh = ret['emesh']
    kline = ret['kline']
    band_plot_module(ax, kline, emesh, n_k1, band1, figname=figname)
    ret = mtb.tb_solver(n_moire, n_g, n_k2, True, symm='d3', rot=0, datatype=datatype, engine=EngineType.TBFULL)
    emesh = ret['emesh']
    kline = ret['kline']
...
def fulltb_combv_cmp(n_moire: int,
                     ...
                     figname=""):
    fig, ax = plt.subplots()
    ret = mtb.tb_solver(n_moire, n_g, n_k1, True, symm='d3', rot=0, datatype=datatype,valley=ValleyType.VALLEYC)
    emesh = ret['emesh']
    kline = ret['kline']
    band_plot_module(ax, kline, emesh, n_k1, band1, figname=figname)
    ret = mtb.tb_solver(n_moire, n_g, n_k2, True, symm='d3', rot=0, datatype=datatype,engine=EngineType.TBFULL)
...
def sparsetb_sepv_cmp(n_moire: int,
    ...
    kline = ret['kline']
    band_plot_module(ax, kline, emesh, n_k1, band1, figname=figname)
    ret = mtb.tb_solver(n_moire, n_g, n_k2, True, symm='d3', rot=0, datatype=datatype,engine=EngineType.TBSPARSE)
    emesh = ret['emesh']
    kline = ret['kline']
    band_plot_module(ax, kline, emesh, n_k2, band2, shape='.', color='red', alpha=0.5, figname=figname)
    plt.tight_layout()
...
#     plt.tight_layout()
#     plt.savefig(pathname+"moire_"+str(n_moire)+"_realspace.png", dpi=500)
#     plt.close()
