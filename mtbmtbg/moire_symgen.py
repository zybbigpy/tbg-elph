...
def symm_c2_z(atom_pstn_2d: np.ndarray) -> np.ndarray:

    return atom_pstn_2d@c2_z
...
def cal_c3_group(n_moire: int, save: bool = False, datatype=DataType.RIGID, path: str = "./") -> tuple:
    ...
    atoms_pstn_list = mio.read_atom_pstn_list(n_moire, datatype)
    n_atoms = atoms_pstn_list.shape[0]
    print("num of atoms", n_atoms)
    ...
    atoms_pstn_2d_c2z = symm_reconstruct(m_basis_vecs, atoms_pstn_2d, symm_c2_z)

    #plt.scatter(atoms_pstn_2d[:, 0], atoms_pstn_2d[:, 1])
    #plt.scatter(atoms_pstn_2d_c2z[:, 0], atoms_pstn_2d_c2z[:, 1])
    #plt.show()
    nn_c31 = find_group_ind(atoms_pstn_2d, atoms_pstn_2d_c31)
    nn_c32 = find_group_ind(atoms_pstn_2d, atoms_pstn_2d_c32)
    nn_c2z = find_group_ind(atoms_pstn_2d, atoms_pstn_2d_c2z)
    ...
    for i in range(n_atoms):
        assert (np.allclose(atoms_pstn_2d[i], atoms_pstn_2d_c2z[nn_c2z[i]]))
    if save:
        np.save(path+datatype+"moire"+str(n_moire)+"_groupc31.npy", nn_c31)
        np.save(path+datatype+"moire"+str(n_moire)+"_groupc32.npy", nn_c32)

    return (nn_c31, nn_c32, nn_c2z)
