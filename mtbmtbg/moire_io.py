...
def read_atom_pstn_list(n_moire: int, datatype=DataType.CORRU) -> np.ndarray:
    ...
    if datatype == DataType.RELAX:
        ...
        #     "../data/relaxsymm/symmatom"+str(n_moire)+"d6-hftn.csv",
        #     delimiter=",",
        #     comments="#",
        # )
        atom_pstn_list = np.loadtxt(
            "../data/relaxsymm/symmatom30-C3-rebo.csv",
            delimiter=",",
            comments="#",
        )
    ...
    elif datatype == DataType.CORRU_D6:
        print("Load D6 corrugation data.")
        atom_pstn_list = np.loadtxt(
            "../data/corrugation_d6/atom"+str(n_moire)+".csv",
            delimiter=",",
            comments="#",
        )
    elif datatype == DataType.PHONON:
        print("Load Electron Phonon data.")
        atom_pstn_list = np.loadtxt(
            "../data/phonon/atom"+str(n_moire)+".csv",
            delimiter=",",
            comments="#",
        )    
